import {useCallback,useEffect,useRef,useState} from "react";import {ArrowLeft,PlugZap,Square} from "lucide-react";import {Link} from "react-router-dom";import {toast} from "sonner";import {Button} from "@/components/ui/button";import {Card,CardContent,CardDescription,CardHeader,CardTitle} from "@/components/ui/card";import {Input} from "@/components/ui/input";import {Label} from "@/components/ui/label";import {textileLabApi} from "@/features/textile-lab/api";import {ArduinoUsbSerialAdapter} from "@/features/textile-lab/adapters/arduinoUsbSerial";import type {ActivityPurpose,Device,LabOverview,RawReading,Specimen,TestSession} from "@/features/textile-lab/types";
const empty:LabOverview={device:null,specimen:null,session:null,latest_reading:null};const purposes:ActivityPurpose[]=["engineering","calibration","controlled_test","real_world_wear","genuine_product_use"];
export default function TextileIntelligenceLab(){const [overview,setOverview]=useState(empty),[devices,setDevices]=useState<Device[]>([]),[specimens,setSpecimens]=useState<Specimen[]>([]),[sessions,setSessions]=useState<TestSession[]>([]),[busy,setBusy]=useState(false),[connected,setConnected]=useState(false),[latest,setLatest]=useState<RawReading|null>(null),[registrationMessage,setRegistrationMessage]=useState<{kind:"success"|"error";text:string}|null>(null);const [deviceForm,setDeviceForm]=useState({device_identifier:"ARDUINO-01",display_name:"Arduino USB Device 01",hardware_type:"microcontroller",hardware_model:"Arduino Nano 33 BLE",firmware_version:"",transport:"usb_serial"});const [sessionForm,setSessionForm]=useState({device_id:"",specimen_id:"",protocol_identifier:"raw-capture-v1",protocol_version:"1.0",test_user_id:"",activity_purpose:"engineering" as ActivityPurpose});const adapter=useRef<ArduinoUsbSerialAdapter|null>(null),abort=useRef<AbortController|null>(null),queue=useRef<RawReading[]>([]),timer=useRef<number|null>(null);
 const load=useCallback(async()=>{try{const [o,d,p,s]=await Promise.all([textileLabApi.overview(),textileLabApi.devices(),textileLabApi.specimens(),textileLabApi.sessions()]);setOverview(o.overview);setDevices(d.devices);setSpecimens(p.specimens);setSessions(s.sessions);setSessionForm(v=>({...v,device_id:v.device_id||d.devices[0]?.id||"",specimen_id:v.specimen_id||p.specimens[0]?.id||""}));}catch(e){toast.error(e instanceof Error?e.message:"Lab data could not be loaded");}},[]);useEffect(()=>{void load();return()=>{abort.current?.abort();if(timer.current)window.clearInterval(timer.current);void adapter.current?.disconnect();}},[load]);
