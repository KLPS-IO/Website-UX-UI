...
export function UploadDialog({ open, setOpen, queue, setQueue, companyId, documents, prefill, afterUploads }: { open: boolean; setOpen: (value: boolean) => void; queue: QueueItem[]; setQueue: React.Dispatch<React.SetStateAction<QueueItem[]>>; companyId: string | null; documents: EvidenceItem[]; prefill: UploadPrefill | null; afterUploads: (linkedCompany: boolean) => Promise<void> }) {
  ...
  const addFiles = (files: File[]) => setQueue((current) => [...current, ...files.map((file): QueueItem => ({ queueId: crypto.randomUUID(), file, title: prefill?.title ?? file.name.replace(/\.[^.]+$/, ""), category: prefill?.category ?? "", documentDate: "", description: prefill?.description ?? "", sourceOrganisation: prefill?.sourceOrganisation ?? "", linkMode: prefill?.linkMode ?? false, entityType: prefill?.entityType ?? "", entityId: prefill?.entityId ?? (prefill?.entityType === "company" ? companyId ?? "" : ""), relationship: prefill?.relationship ?? "", status: "Ready", errors: [], resultMessage: "" }))]);
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>Upload Finance Documents</DialogTitle><DialogDescription>Files are uploaded privately one at a time. Evidence codes, filenames, versions and storage paths are assigned by the backend.</DialogDescription></DialogHeader>
    ...
    {!queue.length && <p className="pb-2 text-center text-sm text-muted-foreground">No files queued.</p>}
    <div className="sticky bottom-0 z-10 -mx-6 mt-2 flex flex-wrap justify-end gap-2 border-t border-border bg-white/95 px-6 pb-5 pt-4 shadow-[0_-8px_18px_rgba(34,40,49,0.06)] backdrop-blur"><button className={`${buttonClass} bg-white`} onClick={() => setOpen(false)}>Close</button>{queue.some((item) => item.status !== "Complete" && item.status !== "Uploading") && <button type="button" aria-label="Upload ready files" className="inline-flex min-w-[190px] items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-[#d98918] bg-[#ef9f32] px-4 py-2 text-sm font-bold text-[#222831] shadow-sm transition hover:bg-[#e39122] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#945c8c] focus-visible:ring-offset-2" onClick={() => void uploadAll()}><Upload className="h-4 w-4 shrink-0" aria-hidden="true" /><span>Upload ready files</span></button>}</div>
