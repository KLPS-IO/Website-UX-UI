...
export function SettingsPage() {
  ...
  return <div><PageIntro eyebrow="Workspace preferences" title="Settings" description="Prepare how Growth OS should work for you as integrations become available." />{notice && <InlineMessage tone="success">{notice}</InlineMessage>}<div className="space-y-4">
    ...
    {[["Connected Accounts", "No accounts connected", "Not connected"],["Future Integrations", "Social, scheduling, AI and analytics", "Planned"]].map(([title,value,status]) => <section key={title} className="flex flex-col gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.035] p-5 sm:flex-row sm:items-center"><div className="flex-1"><h2 className="text-sm font-semibold text-white">{title}</h2><p className="mt-1 text-sm text-white/55">{value}</p></div><span className="text-xs text-[#704293]">{status}</span></section>)}
  </div></div>;
}
