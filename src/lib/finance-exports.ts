...
export async function exportFinanceExcel(data: FinanceExportData) {
  ...
  workbook.creator = "KPLS Ltd";
  workbook.title = "KPLS Financial Report";
  ...
  const summary = addSheet("Executive Summary", [["KPLS FINANCIAL REPORT", ""], ["Status", "Early-stage model — evidence collection in progress"], ["Generated", reportDate(data.generatedAt)], ["Model", "Base case · 18-month horizon"], [], ["Metric", "Value"], ...summaryRows(data)], [36, 54], 6);
  ...
  anchor.href = url;
  anchor.download = `KPLS-Financial-Report-${fileDate(data.generatedAt)}.xlsx`;
...
export async function exportFinancePdf(data: FinanceExportData) {
  ...
  const footer = () => {
    ...
    doc.text("KPLS Investor Financial Report", margin, pageHeight - 8);
  ...
  footer();
  doc.save(`KPLS-Financial-Report-${fileDate(data.generatedAt)}.pdf`);
}
