  ...
  return <div className="space-y-5">
    ...
    {["People","Waitlist","MVP Interest","Champions"].includes(tab) && <section className="rounded-2xl border border-white/10 bg-white/[.025]">
      <div className="grid gap-3 border-b border-white/10 p-4 md:grid-cols-[1fr_auto_auto_auto]">
        <label className="relative"><span className="sr-only">Search people</span><Search className="absolute left-3 top-3.5 h-4 w-4 text-white/40" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search name or email" className="w-full rounded-xl border border-white/15 bg-black/20 py-3 pl-10 pr-3 text-sm text-white placeholder:text-white/35" /></label>
        <select aria-label="Relationship stage" value={stage} onChange={event => { setStage(event.target.value); setPage(1); }} className="rounded-xl border border-white/15 bg-[#151019] px-3 py-3 text-sm text-white"><option value="">All stages</option>{stages.map(item => <option key={item} value={item}>{label(item)}</option>)}</select>
        <select aria-label="Reviewed status" value={reviewed} onChange={event => { setReviewed(event.target.value); setPage(1); }} className="rounded-xl border border-white/15 bg-[#151019] px-3 py-3 text-sm text-white"><option value="">Any review state</option><option value="false">Unreviewed</option><option value="true">Reviewed</option></select>
        <select aria-label="Sort people" value={order} onChange={event => setOrder(event.target.value)} className="rounded-xl border border-white/15 bg-[#151019] px-3 py-3 text-sm text-white"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="next_action_due">Next action due</option></select>
      </div>
